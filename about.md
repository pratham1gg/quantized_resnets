...
Results are saved as individual `result.json` files under `runs/`, organized by experiment type:

- `runs/test_infer/` -- FP32 baselines on PyTorch and TensorRT (84 runs: 4 input bit-widths x 7 backend-precision combos x 3 seeds)
- `runs/val_infer/` -- validation set runs, mostly CPU PTQ experiments (88 runs)
- `runs/qat_test/` -- QAT models evaluated on the test set (12 runs)
- `runs/qat_trt_test/` -- QAT models exported to TensorRT and evaluated (4 runs)
...
- `src/config.py` -- the `ExperimentConfig` dataclass. Every experiment is defined by one of these: which backend, what precision, how many input bits, which seed, etc. It also knows how to generate a `run_id` string and stamp system info.
...
- `qat_training.py` -- takes a trained FP32 checkpoint and fine-tunes it with QAT using ModelOpt. 15 epochs, small learning rate (1e-4). Inserts Q/DQ nodes for INT8 or INT4 weight simulation.
- `run_multiseed_fp32.sh` / `run_multiseed_lowbit.sh` / `run_qat.sh` / `run_qat_int4.sh` / `run_qat_multiseed.sh` -- shell scripts that launch training across multiple seeds and configurations. These are what you'd actually run to reproduce all the checkpoints.
...
### `runs/` -- raw experiment results (188 individual runs)

Every single experiment produces a `result.json` in its own folder. The folder name encodes the config (e.g., `resnet18_tensorrt_int8_in2b_cuda_bs1`).

- `test_infer/fp32_{1,2,4,8}bit/seed_{1,2,42}/` -- FP32 baseline inference on the test set. 7 backend-precision combos (PyTorch FP32, PyTorch FP16, TRT FP32, TRT FP16, TRT FP8, TRT INT8, TRT INT4) x 4 input bit-widths x 3 seeds = 84 runs.
- `val_infer/` -- validation set inference, including the CPU PTQ INT8 experiments. 88 runs.
- `qat_test/` -- QAT models evaluated on test data via PyTorch CUDA and TRT. 12 runs.
- `qat_trt_test/` -- QAT INT8 models specifically on TRT. 4 runs.
...
- `01_fp32_baselines_sweep.ipynb` -- run FP32 baselines across all configs
- `01_1_pytorch_test.ipynb` -- PyTorch-only test inference
- `02_tensorrt_inference.ipynb` -- TensorRT inference sweep
- `02_1_tensorrt_test.ipynb` -- TensorRT test set evaluation
- `03_1_qat_test.ipynb` -- QAT model evaluation
- `04_ptq_test.ipynb` -- PTQ experiments
- `05_qat_test.ipynb` -- additional QAT testing
- `06_qat_trt_test.ipynb` -- QAT models on TensorRT
- `50_ptq_int8_cpu.ipynb` -- CPU-only PTQ INT8 experiment
- `analysisv2.ipynb` -- main analysis and plotting notebook
...
- `delete/` -- stuff marked for removal (old `results/` folder, empty `qat_int4act_test/` directory)
