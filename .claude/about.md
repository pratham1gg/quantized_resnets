...
- `final/` -- clean CSVs averaged across seeds, used by the plotting notebooks:
  - `avg_ptq.csv` -- one row per (backend, precision, input_bits) with mean and std for top-1, top-5, and latency.
  - `avg_qat.csv` -- same structure for QAT experiments.
  - `raw_ptq.csv`, `raw_qat.csv` -- per-seed rows before averaging.
...
- `plots_pgf/` -- PGF format plots for LaTeX documents (generated by `plots_pgf.ipynb`).
...
- `final_results.ipynb` -- aggregates raw `result.json` files into the CSVs in `resultsv2/final/`
- `analysis.ipynb` -- main analysis tables
- `plots.ipynb` -- interactive result plots (13 plots with `plt.show()`, no file output)
- `plots_pgf.ipynb` -- same plots rendered with matplotlib's PGF backend for LaTeX-quality fonts, saves `.pgf` files to `resultsv2/plots_pgf/`
